...
def invalidate_user_session(db: Session, session_id: int) -> bool:
    ...
    s.is_active = False
    s.logout_time = now
    log = (db.query(models.LoginLog)
           .filter(models.LoginLog.user_id == s.user_id, models.LoginLog.login_status == "Success",
                   models.LoginLog.logout_time.is_(None))
           .order_by(models.LoginLog.login_time.desc()).first())
    if log:
        log.logout_time = now
    db.commit()
    return True
...
def count_learning_history(db: Session, user_id: int, activity_type: str | None = None) -> int:
...
def get_weekly_activity(db: Session, user_id: int) -> list[dict]:
    """Return exactly seven days of activity for the Profile weekly chart."""
    today = datetime.now(timezone.utc).date()
    start = today - timedelta(days=6)
    rows = (db.query(models.LearningHistory)
            .filter(models.LearningHistory.user_id == user_id,
                    models.LearningHistory.completed_at >= datetime.combine(start, datetime.min.time(), tzinfo=timezone.utc))
            .all())
...
def _refresh_streak(db: Session, user_id: int, stats: models.UserStatistics) -> None:
    dates = (db.query(models.LearningHistory.completed_at)
             .filter(models.LearningHistory.user_id == user_id)
             .order_by(models.LearningHistory.completed_at.desc()).all())
    unique_dates = set()
...
def calculate_ai_reading_score(db: Session, reading: models.AIReading) -> models.AIReading:
    ...
    duration = _duration_minutes(reading.generated_at, reading.completed_at)

    _update_statistics_after_reading(db, reading.user_id, reading.accuracy)
    stats = get_user_statistics(db, reading.user_id)
    if stats and duration is not None:
        stats.study_hours = round(stats.study_hours + duration / 60.0, 2)

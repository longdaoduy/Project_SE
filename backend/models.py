...
Tables (in dependency order):
  ...
  user_card_srs       – FR2  Per-(user,word) SRS state (persists between sessions)
  daily_learning_log  – FR2  Daily new-word introduction tracker
...
class UserCardSRS(Base):
    """
    Per-(user, word) Spaced Repetition state.
    Persists between sessions so the scheduler always knows when to show a card next.

    card_status:
      'new'      – never studied yet (introduced today via daily limit)
      'learning' – rated Again recently, show again within same day
      'review'   – graduated; due_date is set days/weeks in the future
    """
    __tablename__ = "user_card_srs"

    srs_id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(
        ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False, index=True
    )
    word_id: Mapped[int] = mapped_column(
    ...
    card_status: Mapped[str] = mapped_column(
        Enum("new", "learning", "review", name="card_status_enum"),
        nullable=False, default="new"
    )
    due_date: Mapped[str] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), index=True
    )
    last_reviewed: Mapped[str | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class DailyLearningLog(Base):
    """
    Tracks which new words were introduced per (user, topic, date).
    Used to enforce the DAILY_NEW_LIMIT cap (default 15 new words/topic/day).
    One row per (user_id, topic_id, word_id, learned_at) – unique constraint
    prevents double-counting if the client retries.
    """
    ...
    learned_at: Mapped[str] = mapped_column(Date, nullable=False, index=True)
...
class AIReading(Base):
    ...
    title              : short descriptive title generated by AI (e.g. "Travel – B1")
    time_limit_seconds : fixed countdown per difficulty (set on creation)
    attempt_number     : 1 for first attempt, 2+ for retakes
    parent_reading_id  : for retakes – points to the original reading so the
                         same passage/questions are reused without regeneration
    completion_seconds : how many seconds the user actually took (≤ time_limit_seconds)
    """
    ...
    # Timer fields
    time_limit_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=600)
    completion_seconds: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # Retake linkage
    attempt_number: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    parent_reading_id: Mapped[int | None] = mapped_column(
        ForeignKey("ai_readings.reading_id", ondelete="SET NULL"), nullable=True, index=True
    )
...
class AIReadingQuestion(Base):
    ...
    explanation : generated by AI after the test is submitted – stored once,
                  reused on every retake so the AI is not called again.
    """
